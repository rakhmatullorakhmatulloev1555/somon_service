...
def assign_master_by_telegram(ticket_id: int, master_telegram_id: int):
    ...
    try:
        ...
        if ticket.master_id:
            return False, "На эту заявку уже назначен мастер"
...
def assign_master_by_id(ticket_id: int, master_id: int):
    """Назначить мастера на заявку по ID мастера"""
    db = SessionLocal()
    try:
        ticket = db.query(Ticket).filter(Ticket.id == ticket_id).first()
...
def get_master_tickets(master_telegram_id: int):
    """Получить заявки мастера по его Telegram ID"""
    db = SessionLocal()
    try:
        master = db.query(Master).filter(Master.telegram_id == str(master_telegram_id)).first()
        ...
        print(f"Error getting master tickets: {e}")
        return []
    finally:
        db.close()
